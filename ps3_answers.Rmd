...
```{r}
set.seed(rand_seed)
gp_samples <- gaussian_process_sampler(noise_sd=0.0000001,length_scale=1,signal_sd=1)
plot(gp_samples)
```
...
# y <- branin(x_1,x_2)$output # function to learn
X <- cbind(x_1,x_2) # training points
# Test points:
X_test <- as.matrix(
  expand.grid( 
    x_1 = seq(min(x_lim),max(x_lim),length.out=n_test),
    x_2 = seq(min(x_lim),max(x_lim),length.out=n_test)
  )
)
```
